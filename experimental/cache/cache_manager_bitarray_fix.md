...
class CacheScopeManager:
    ...
    def _write_id_mappings(self, mappings_file: Path, node_mapping: Dict[str, int], link_mapping: Dict[str, int]) -> int:
        ...
        try:
            # Write JSON to gzipped file (gzip.open handles compression automatically)
            with gzip.open(temp_file, 'wt', encoding='utf-8', compresslevel=6) as f:
                json.dump(mappings_data, f, separators=(',', ':'))  # Compact JSON format
    ...
    def _read_id_mappings(self, mappings_file: Path) -> Tuple[Dict[str, int], Dict[str, int]]:
        ...
        # gzip.open automatically decompresses when reading
...
The methods are **correct**! Here's how they work:

## `_write_id_mappings` Flow:
1. **Python dict → JSON conversion**: `json.dump()` converts the Python dictionaries to JSON format
2. **Compression**: `gzip.open(file, 'wt')` automatically compresses the JSON text as it's written
3. **File extension**: `.json.gz` indicates it's a gzipped JSON file

## `_read_id_mappings` Flow:
1. **Decompression**: `gzip.open(file, 'rt')` automatically decompresses the file as it's read
2. **JSON → Python dict**: `json.load()` parses the JSON back into Python dictionaries

## The Process:
```python
# Writing:
Python Dict → json.dump() → JSON text → gzip.open('wt') → Compressed file

# Reading:  
Compressed file → gzip.open('rt') → JSON text → json.load() → Python Dict
```

## Why This Works Well:
- **ID mappings compress extremely well**: Node/link IDs often have patterns (sequential numbers, prefixes, etc.)
- **JSON is human-readable**: Easy to debug if needed
- **gzip.open() handles everything**: No manual compression/decompression steps
- **Text mode ('wt'/'rt')**: Handles encoding automatically

## File Size Example:
```python
# Raw mappings: 4M nodes + 8M links = ~12M entries
# Uncompressed JSON: ~200-400MB  
# Compressed JSON: ~20-50MB (80-90% compression typical for ID mappings)
```

The `.json.gz` extension makes it clear that it's compressed JSON, and `gzip.open()` handles the compression/decompression transparently!
